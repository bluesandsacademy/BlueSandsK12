...
// blackboard has no published price yet, and the tablet is a per-student annual
// subscription quoted in USD. Both cases need their own Offer rather than the
// NGN one-off `productSchema` builds.
export function solutionSchema(slug) {
  ...
  const offer =
    s.price.mode === "subscription"
      ...
      : {
          // No figure published: PreOrder with an unknown price is the honest
          // signal, and a fabricated number would be a rich-result violation.
          "@type": "Offer",
          url: `${SITE_URL}/products/${s.slug}`,
