...
// blackboard has no published price yet, and the tablet is bundled free into
// every AR Book kit rather than sold on its own. None of that is a NGN
// one-off, so each mode gets its own Offer rather than the shape
// `productSchema` builds.
export function solutionSchema(slug) {
  ...
  const offer =
    s.price.mode === "subscription"
      ...
      : s.price.mode === "bundled"
        ? {
            // Genuinely available, just not sold as its own line item: no
            // price/currency published, same as the honest "no figure" case.
            "@type": "Offer",
            url: `${SITE_URL}/products/${s.slug}`,
            availability: "https://schema.org/InStock",
            seller: { "@id": ORG_ID },
...
export const solutionMetaTitle = (s) =>
  s.price.mode === "subscription"
    ? `${s.name}: $${s.price.usd} per student, per year`
    : s.price.mode === "bundled"
      ? `${s.name}: Included with Every AR Book`
