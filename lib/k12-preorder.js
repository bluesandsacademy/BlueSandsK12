...
export const PREORDER_PACKAGES = [
  ...
  },
  {
    ...
    // This package is priced through TABLET_OPTIONS below, not here. These two
    // fields are the fallback / card headline (the "with tablet" figure).
    priceUSD: 100,
    priceNGN: 160_000,
...
// Each option carries its own price and unit: buying the tablet is a one-time
// payment, bringing your own device is a per-student yearly subscription.
export const TABLET_OPTIONS = [
  {
    id: "with",
    label: "With Smart Tablet",
    description:
      "Get a dedicated Smart Tablet for a one-time payment of ₦160,000, with access to our STEM simulations.",
    priceNGN: 160_000,
    priceUSD: 100,
    unit: "one-time",
  },
  {
    ...
    description:
      "Use your own tablet and pay ₦50,000 per student each year for a subscription to the virtual STEM simulations.",
    priceNGN: 50_000,
    priceUSD: 30,
...
/**
 * List price for a package, in both currencies, plus the unit it is charged in.
 * The tablet package is priced by its `tabletOption` (defaulting to "with").
 */
export function packagePrice(pkg, tabletOption) {
  if (!pkg) return { usd: 0, ngn: 0, unit: null };
  if (pkg.hasTabletOption) {
    const opt =
      TABLET_OPTIONS.find((o) => o.id === tabletOption) || TABLET_OPTIONS[0];
    return { usd: opt.priceUSD, ngn: opt.priceNGN, unit: opt.unit };
  }
  return { usd: pkg.priceUSD, ngn: pkg.priceNGN, unit: "per kit" };
}
