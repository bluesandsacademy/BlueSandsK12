...
// display and a per-student annual subscription fit none of that, so they are
...
// instead of a number. The Virtual Science Lab Tablet has two purchase tiers,
// depending on whether Blue Sands supplies the tablet hardware or the school
// brings its own device. Either tier includes the first year of the Virtual
// Science Labs subscription free; from year two it renews annually at the
// lower TABLET_RENEWAL_USD rate. All three figures are authored in USD only;
// there is no authored NGN, so this is NOT run through <Price>.
// ─────────────────────────────────────────────────────────────────────────────
export const TABLET_TIER_WITH_USD = 150;
export const TABLET_TIER_WITHOUT_USD = 100;
export const TABLET_RENEWAL_USD = 75;
...
export const solutions = [
...

  {
    ...
    blurb:
      "A virtual science laboratory for Secondary Schools with over 250 simulations every student can practice.",
    description:
    ...
    price: {
      ...
      tiers: [
        {
          key: "with-tablet",
          label: "With Smart Tablet",
          usd: TABLET_TIER_WITH_USD,
        },
        {
          key: "without-tablet",
          label: "Without Smart Tablet",
          usd: TABLET_TIER_WITHOUT_USD,
        },
      ],
      renewalUsd: TABLET_RENEWAL_USD,
      unit: "Per student · per school",
      note: `Priced in USD. $${TABLET_TIER_WITH_USD} includes the Smart Tablet; $${TABLET_TIER_WITHOUT_USD} if your school brings its own device instead. Either way, your first year of Virtual Science Labs is included. From year two, renews at $${TABLET_RENEWAL_USD} per student, per year. Contact us for the naira equivalent and school-wide quantities.`,
    },

    heroHighlights: [
      { value: "27", label: "Topic areas across 4 sciences" },
      { value: "9", label: "Subjects in the textbook library" },
      { value: "$150", label: "Full package, tablet included" },
    ],

    faqs: [
      {
        q: "What's the difference between the two prices?",
        a: "$150 includes a Smart Tablet supplied by Blue Sands. $100 is the same learning platform, Virtual Science Labs, STEM simulations, digital textbooks, Learning Management System, teacher dashboard, assessments and support, running on a compatible device your school already owns. Both include your first year free; from year two, renewal is $75 per student, per year.",
      },
      {
    ...
    // `tabletOnly` marks the one line that the without-tablet tier drops;
    // every other line runs the same on a school's own device.
    checklist: {
      Icon: ListChecks,
      kicker: "What's included",
      title: "With tablet vs. without",
      items: [
        { label: "Smart Tablet device", tabletOnly: true },
        { label: "Learning platform software" },
        { label: "Virtual Science Labs" },
        { label: "Interactive STEM simulations" },
        { label: "AI Learning Assistant" },
        { label: "Digital textbooks" },
        { label: "Learning Management System access" },
