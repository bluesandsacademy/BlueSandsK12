...
// The platform pre-order takes no payment. On submission we send the school an
// acknowledgement and the admin inbox an alert with a link into the admin.

export async function sendK12PreorderAcknowledgement({ to, contactPerson, orgName, packageLabel, studentLicenses, preorderId }) {
  const ref = `BSL-${String(preorderId).slice(0, 8).toUpperCase()}`;
  return resend.emails.send({
    from: FROM,
    to,
    subject: `Pre-order request received | Blue Sands K12`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#02345a">
        <div style="background:#02345a;padding:24px 32px;border-radius:12px 12px 0 0">
          <h1 style="color:white;margin:0;font-size:22px">Blue Sands K12</h1>
          <p style="color:rgba(255,255,255,0.7);margin:4px 0 0">Pre-Order Request Received</p>
        </div>
        <div style="background:#fff;padding:32px;border:1px solid #e5e7eb;border-radius:0 0 12px 12px">
          <p style="font-size:18px;font-weight:bold;margin-top:0">Hello ${esc(contactPerson)},</p>
          <p>Thanks for your interest in Blue Sands K12 for <strong>${esc(orgName)}</strong>. Your pre-order request is in. Here is what you sent:</p>
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            <tr style="background:#f9fafb"><td style="padding:10px;border:1px solid #e5e7eb;font-weight:600">Package</td><td style="padding:10px;border:1px solid #e5e7eb">${esc(packageLabel)}</td></tr>
            <tr><td style="padding:10px;border:1px solid #e5e7eb;font-weight:600">Student licenses</td><td style="padding:10px;border:1px solid #e5e7eb">${esc(studentLicenses)}</td></tr>
            <tr style="background:#f9fafb"><td style="padding:10px;border:1px solid #e5e7eb;font-weight:600">Reference</td><td style="padding:10px;border:1px solid #e5e7eb">${ref}</td></tr>
          </table>
          <p>Our team will review your requirements and contact you with pricing, package options, and next steps. Submitting this form is a pre-order request, not a payment or a binding purchase.</p>
          <p style="color:#555">Questions? Reply to this email or reach us on WhatsApp.</p>
          <p style="margin-bottom:0">The Blue Sands STEM Labs Team</p>
        </div>
      </div>
...
export async function sendK12AdminPreorderAlert({ orgName, contactPerson, jobTitle, email, phone, location, packageLabel, studentLicenses, studentCount, preorderId }) {
  const ref = `BSL-${String(preorderId).slice(0, 8).toUpperCase()}`;
  return resend.emails.send({
    from: FROM,
    to: ADMIN_INBOX,
    replyTo: email,
    subject: `[K12 Platform] New Pre-Order Request: ${orgName}`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#02345a">
        <div style="background:#02345a;padding:20px 32px;border-radius:12px 12px 0 0">
          <h1 style="color:white;margin:0;font-size:18px">New K12 Platform Pre-Order</h1>
        </div>
        <div style="background:#fff;padding:28px 32px;border:1px solid #e5e7eb;border-radius:0 0 12px 12px">
          <table style="width:100%;border-collapse:collapse">
            <tr style="background:#f9fafb"><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600;width:38%">School / Org</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(orgName)}</td></tr>
            <tr><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Contact</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(contactPerson)}, ${esc(jobTitle)} &lt;${esc(email)}&gt;</td></tr>
            <tr style="background:#f9fafb"><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Phone / WhatsApp</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(phone)}</td></tr>
            <tr><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Location</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(location)}</td></tr>
            <tr style="background:#f9fafb"><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Package</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(packageLabel)}</td></tr>
            <tr><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Student licenses</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(studentLicenses)}</td></tr>
            <tr style="background:#f9fafb"><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Approx. students</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${esc(studentCount)}</td></tr>
            <tr><td style="padding:10px 12px;border:1px solid #e5e7eb;font-weight:600">Reference</td><td style="padding:10px 12px;border:1px solid #e5e7eb">${ref}</td></tr>
          </table>
          <div style="margin-top:20px;text-align:center">
            <a href="${SITE_URL}/admin/k12-preorders/${preorderId}" style="background:#0483e2;color:white;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:bold;font-size:14px;display:inline-block">View in Admin</a>
          </div>
        </div>
      </div>
    `,
