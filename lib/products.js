...
// a customer can put a kit AND a spare tablet in ONE cart and check out once —
// Paystack buy links are single-product and can't do that. The store lists
// all products on one page, so landing on its home is a fine entry.
const STORE_URLS = {
...
// A Smart Tablet ships in every kit (see `baseKit` below); TABLET_NGN/USD
// here price the standalone spare/replacement listing only.
...
// ─────────────────────────────────────────────────────────────────────────────
// Two authored prices per item. NGN is what Paystack actually charges (mirror
// it in the Paystack product). USD is a fixed, clean headline price — NOT a live
// conversion — shown as the leading number. They are set independently; if you
// reprice one, decide the other by hand.
export const TABLET_NGN = 70_000; // reference price — a Smart Tablet ships in every kit (see baseKit); kept for the standalone/replacement listing
export const TABLET_USD = 50;
...
// A standalone Smart Tablet listing, for a replacement or a spare, sold as
// its own Paystack product. It lives in the same store as the kits, so the
// buy buttons that route to STORE_URL already reach it — no separate link is
// wired. Direct buy links kept here for reference only.
export const TABLET = {
  name: "Smart Tablet",
  priceNGN: TABLET_NGN,
  priceUSD: TABLET_USD,
...
// Every kit ships with a Smart Tablet, the camera holder, markers and the
// app; only the books differ. The book line is prepended per product in
// `package`.
const baseKit = [
  {
    name: "Smart Tablet",
    detail: "Included with every kit, ready to scan the pages",
    Icon: Tablet,
  },
  { name: "Spotty Camera", detail: "Smart holder for the tablet" },
  {
    name: "Marker Set",
